...
{
	new_full_parts(config, telemetry, executor, pruning_filters, tx_priority_modifier).map(|parts| parts.0)
}
...
where
	...
	TExec: CodeExecutor + RuntimeVersionOf + Clone,
{
	new_full_parts_record_import(config, telemetry, executor, false, pruning_filters, tx_priority_modifier)
}
