...
pub trait VotingHooks<AccountId, Index, Balance> {
    ...
    fn on_remove_unsuccessful_vote(who: &AccountId, ref_index: Index) -> Result<Balance, DispatchError>;
...
impl<A, I, B> VotingHooks<A, I, B> for () {
    ...
    fn on_remove_unsuccessful_vote(_who: &A, _ref_index: I) -> Result<B, DispatchError> {
        Ok(Default::default())
    }
